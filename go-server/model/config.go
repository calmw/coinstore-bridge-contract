...
type Config struct {
	...
	FreshStart         int    `gorm:"fresh_start;comment:'启动时，是否从设置的起始块高开始'" json:"fresh_start"`
	...
	GasLimit           string `gorm:"gas_limit;comment:''" json:"gas_limit"`
	MaxGasPrice        string `gorm:"max_gas_price;comment:''" json:"max_gas_price"`
	MinGasPrice        string `gorm:"min_gas_price;comment:''" json:"min_gas_price"`
	Http               int    `gorm:"http;comment:'http or ws'" json:"http"`
	StartBlock         string `gorm:"start_block;comment:'起始块高'" json:"start_block"`
	BlockConfirmations string `gorm:"block_confirmations;comment:'待确认块高差'" json:"block_confirmations"`
}
